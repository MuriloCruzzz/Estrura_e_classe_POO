...
int main(void)
{
  ...
  struct CAD_ALUNO // criando uma estrutura para definir os campos
    {
      char  NOME[41]; // caracter com 41 posições tipo de dado pré formatado devido ao registro
      float NOTA[4]; // float com 4 posições tipo de dado pré formatado devido ao registro
      float MEDIA; // média tipo de dado pré formatado devido ao registro
    };

  CAD_ALUNO ALUNO[8], X; // define a quantidade de alunos
  int I, J; // 
  float SOMA_NT, SOMA_MD, MEDIA_GP;// 
  char ENTRANOTA[6];//

  cout << setprecision(2); // formatação
  ...
  for (I = 0; I <= 7; I++)
    {
      ...
      cin.getline(ALUNO[I].NOME, sizeof(ALUNO[I].NOME)); // ° sizeof
      cout << endl;
      cout << "Entre as notas:\n" << endl;
      for (J = 0; J <= 3; J++)
        {
